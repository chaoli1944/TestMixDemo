...
import com.tencent.live.utils.MixTranscodeUtils;
...
public abstract class LiveBaseActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public int mixMode = 3;
    private boolean isMixTransCodeManual = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(mixMode == TRTCCloudDef.TRTC_TranscodingConfigMode_Template_PresetLayout){
//            MixTranscodeUtils.mixTransCodeAuto(mTRTCCloud,mRoomId);
        }else if(mixMode == TRTCCloudDef.TRTC_TranscodingConfigMode_Manual){
            isMixTransCodeManual = true;
        }
    ...
    protected void initView() {
        ...
        mLinkMicSelfPreviewView     = findViewById(R.id.live_cloud_view_0); // 位置1画面View
...
//        mRemoteViewList = new ArrayList<>(DEFAULT_CAPACITY);
        mRemoteViewList = new LinkedList<>();
    ...
    private void handleIntent() {
        ...
        if (null != intent) {
            ...
            if (intent.getIntExtra(Constant.MIX_MODE, 0) != 0) {
                mixMode = intent.getIntExtra(Constant.MIX_MODE, 0);
            }
        }
    }
    ...
    public class TRTCCloudImplListener extends TRTCCloudListener {
        ...
        public void onSetMixTranscodingConfig(int i, String s) {
            ...
            Log.i("chaoli", "onSetMixTranscodingConfig: code="+i+";msg="+s);
        }
...
//        }
    ...
    public void refreshRemoteVideoViews() {
        for (int i = 0; i < mRemoteViewList.size(); i++) {
            if (i < mRemoteUidList.size()) {
                ...
                // 开始显示用户userId的视频画面
                mTRTCCloud.startRemoteView(remoteUid, mRemoteViewList.get(i).getVideoView());
                mTRTCCloud.setRemoteViewFillMode(remoteUid,TRTCCloudDef.TRTC_VIDEO_RENDER_MODE_FILL);
            } else {
                mRemoteViewList.get(i).setVisibility(View.GONE);
            }
        }
    ...
    public boolean isAnchor(){
        ...
        boolean isAnchor = mRoomId.equals(mUserId)? true:false;
        return isAnchor;
    }
    ...
    protected class LiveSubViewListenerImpl implements LiveSubVideoView.LiveSubViewListener {
        ...
        public void onMuteRemoteVideoClicked(View view) {
            boolean isSelected = view.isSelected();
            int size = mRemoteUidList.size();
            Log.i(TAG, "onMuteRemoteVideoClicked: size="+size+"; mIndex="+mIndex);
            if (!isSelected) {
